//! Loading and updating `dxclusters.dat` — the public DX cluster node list
//! offered as connection presets.
//!
//! Mirrors `cty_update.rs`: a downloaded copy in the app data dir wins over the
//! bundled resource. [`maybe_update`] refreshes it weekly with a conditional
//! GET (`If-None-Match` from a saved ETag), so an unchanged file costs one 304.

use std::path::{Path, PathBuf};
use std::time::Duration;

use dxcluster_core::reference::{parse_presets, ClusterPreset};
use serde::Serialize;
use tauri::{AppHandle, Manager};

// TODO: switch to raw.githubusercontent.com once the repo is public.
const PRESETS_URL: &str = "https://dacr.hu/DXCLUSTERS.DAT";
const MAX_AGE: Duration = Duration::from_secs(7 * 24 * 3600);

/// Result of a preset-list update / status check, surfaced to the UI.
#[derive(Debug, Clone, Serialize)]
pub struct PresetsStatus {
    pub count: usize,
    /// `"downloaded"`, `"bundled"`, or `"none"`.
    pub source: String,
    pub age_days: Option<u64>,
    pub version: Option<String>,
    pub updated: bool,
}

fn local_path(app: &AppHandle) -> PathBuf {
    app.path()
        .app_data_dir()
        .unwrap_or_else(|_| PathBuf::from("."))
        .join("dxclusters.dat")
}

fn etag_path(app: &AppHandle) -> PathBuf {
    local_path(app).with_extension("dat.etag")
}

fn file_age(path: &Path) -> Option<Duration> {
    std::fs::metadata(path)
        .ok()?
        .modified()
        .ok()?
        .elapsed()
        .ok()
}

/// Load the preset list, preferring a downloaded copy over the bundled resource.
/// Returns `(nodes, version, source)`.
pub fn load(app: &AppHandle) -> (Vec<ClusterPreset>, Option<String>, &'static str) {
    if let Ok(text) = std::fs::read_to_string(local_path(app)) {
        let (nodes, version) = parse_presets(&text);
        if !nodes.is_empty() {
            log::info!("loaded {} downloaded cluster presets", nodes.len());
            return (nodes, version, "downloaded");
        }
    }
    let bundled = app
        .path()
        .resolve(
            "resources/dxclusters.dat",
            tauri::path::BaseDirectory::Resource,
        )
        .ok()
        .and_then(|p| std::fs::read_to_string(p).ok());
    match bundled {
        Some(text) => {
            let (nodes, version) = parse_presets(&text);
            log::info!("loaded {} bundled cluster presets", nodes.len());
            (nodes, version, "bundled")
        }
        None => {
            log::warn!("no dxclusters.dat found; no connection presets");
            (Vec::new(), None, "none")
        }
    }
}

/// Current status without touching the network.
pub fn status(
    app: &AppHandle,
    count: usize,
    version: Option<String>,
    source: &str,
) -> PresetsStatus {
    PresetsStatus {
        count,
        source: source.to_string(),
        age_days: file_age(&local_path(app)).map(|d| d.as_secs() / 86_400),
        version,
        updated: false,
    }
}

/// Download when the local copy is missing or older than a week.
pub async fn maybe_update(app: &AppHandle) -> Result<bool, String> {
    let stale = file_age(&local_path(app))
        .map(|a| a > MAX_AGE)
        .unwrap_or(true);
    if stale {
        force_update(app).await
    } else {
        Ok(false)
    }
}

/// Fetch the list now (conditional on the saved ETag). Returns whether it changed.
pub async fn force_update(app: &AppHandle) -> Result<bool, String> {
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(30))
        .build()
        .map_err(|e| e.to_string())?;

    let mut req = client.get(PRESETS_URL);
    if let Ok(etag) = std::fs::read_to_string(etag_path(app)) {
        let etag = etag.trim();
        if !etag.is_empty() {
            req = req.header(reqwest::header::IF_NONE_MATCH, etag);
        }
    }

    let resp = req
        .send()
        .await
        .map_err(|e| format!("download: {e}"))?
        .error_for_status()
        .map_err(|e| format!("HTTP: {e}"))?;

    if resp.status() == reqwest::StatusCode::NOT_MODIFIED {
        // Touch the file so we don't re-check for another week.
        let _ = filetime_touch(&local_path(app));
        return Ok(false);
    }

    let new_etag = resp
        .headers()
        .get(reqwest::header::ETAG)
        .and_then(|v| v.to_str().ok())
        .map(str::to_string);
    let body = resp.text().await.map_err(|e| format!("read: {e}"))?;

    // Sanity check: must parse to a plausible node list.
    let (nodes, _) = parse_presets(&body);
    if nodes.len() < 100 {
        return Err(format!(
            "downloaded file looks wrong ({} nodes)",
            nodes.len()
        ));
    }

    let local = local_path(app);
    if let Some(dir) = local.parent() {
        std::fs::create_dir_all(dir).ok();
    }
    let changed = std::fs::read_to_string(&local)
        .map(|old| old.trim() != body.trim())
        .unwrap_or(true);
    std::fs::write(&local, body.as_bytes()).map_err(|e| format!("write: {e}"))?;
    match new_etag {
        Some(e) => {
            let _ = std::fs::write(etag_path(app), e);
        }
        None => {
            let _ = std::fs::remove_file(etag_path(app));
        }
    }
    Ok(changed)
}

/// Bump a file's mtime to now (best effort) by rewriting it in place.
fn filetime_touch(path: &Path) -> std::io::Result<()> {
    let data = std::fs::read(path)?;
    std::fs::write(path, data)
}
